...
def get_geolb_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    dataset, expected_len = get_RGB_dataset(dataset_root=args.train_data,
        feature_models = [],
        dataset_mix = args.datamix.split(","),
        dataset_ratio = 1.0,
    ...
    num_batches = math.ceil(N_samples / args.batch_size / args.world_size)
    #num_batches = N_samples // args.batch_size
    #logger.debug(f"--{N_samples}, batch size: {args.batch_size}, num batches:{num_batches}")

    image_loader.num_batches = num_batches
    image_loader.num_samples = expected_len

    return DataInfo(dataloader=image_loader, shared_epoch=shared_epoch)
